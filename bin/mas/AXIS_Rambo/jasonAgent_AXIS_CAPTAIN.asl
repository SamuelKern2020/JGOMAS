...
+!setup_priorities
    <-  +task_priority("TASK_NONE",0);
        ...
        +task_priority("TASK_GOTO_POSITION2", 749);
...
+!update_targets 
	<-	?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR UPDATE_TARGETS GOES HERE.") };
		...
		if(TIME == 0){
			-+time(1);
			.concat("captainpos(",30,",0,",210,")",Captain_Position);
			.send_msg_with_conversation_id(AxisTeam, tell, Captain_Position, "FMT");
			!add_task(task(3000,"Task_GOTO_POSITION",ME,pos(30,0,210),""));
		}
		else{
			if(TIME==1 & FORMED==7 & TODOSFORMADOS=false){
				.concat("captainpos(",30,",0,",60,")",Captain_Position);
				.send_msg_with_conversation_id(AxisTeam, tell, Captain_Position, "FMT");
				-+formed(0);
				-+time(2);
				!add_task(task(3000,"Task_GOTO_POSITION",ME,pos(30,0,60),""));
			}
			else{
				if(TIME==2 & FORMED==7 & TODOSFORMADOS=false){
					BaseENEMIGA = "base(true)";
					-+formed(0);
					.send_msg_with_conversation_id(AxisTeam, tell, BaseENEMIGA , "BAS");
					.concat("captainpos(",30,",0,",55,")",Captain_Position);
					.send_msg_with_conversation_id(AxisTeam, tell, Captain_Position , "BAS");
					-+time(3);
					!add_task(task(3000,"Task_GOTO_POSITION",ME,pos(30,0,54),""));
					!add_task(task(2999,"Task_GOTO_POSITION2",ME,pos(30,0,55),""));	
				}
				else{
					if(TIME==3 & FORMED==7 & TODOSFORMADOS=false){
						-+todos_formados(true);
						.print("EN POSICION");
					}
				}
