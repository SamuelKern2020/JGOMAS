...
+!update_targets 
	...
	.my_team("AXISCAPTAIN",CAPTAIN);
	?time(TIME);
	?posObjective(OBJECTIVEX,0,OBJECTIVEZ);
	?my_position(MYX,MYY,MYZ);
	MX = math.floor(MYX);
	MZ = math.floor(MYZ);
	if(TIME==1 & MX==OBJECTIVEX & MZ==OBJECTIVEZ){
		Soldier_Arrived = "ready(1)";
		.send_msg_with_conversation_id(CAPTAIN, tell, Soldier_Arrived, "OKE");}
	else{
		if(TIME==2 & MX==OBJECTIVEX & MZ==OBJECTIVEZ){
			Soldier_Arrived = "ready(2)";
			.send_msg_with_conversation_id(CAPTAIN, tell, Soldier_Arrived, "OKO");}
		else{
			if(TIME==3 & MX==OBJECTIVEX & MZ==OBJECTIVEZ){
				Soldier_Arrived = "ready(3)";
				.send_msg_with_conversation_id(CAPTAIN, tell, Soldier_Arrived, "OKA");
			}	
		}
	}.
...
+!performThresholdAction
       ...
       ?my_ammo(Ar);
       // Los paquetes de municion los da el mas cercano //
       if (Ar <= At) { 
         ?my_position(X, Y, Z);        
         .my_team("fieldops_AXIS", E1);
         !nearest(E1);
         ?nearest(NearestIntendence,_,_);
         .concat("cfa(",X, ", ", Y, ", ", Z, ", ", Ar, ")", Content1);
         .send_msg_with_conversation_id(NearestIntendence, tell, Content1, "CFA");
       ...
       ?my_health(Hr);
       // Los paquetes de medicina los da el mas cercano //
       if (Hr <= Ht) { 
         ?my_position(X, Y, Z);         
...
/////////////////////////////////
//   Captain position belief   //
/////////////////////////////////

+captainpos(CAPTAINPOSX,0,CAPTAINPOSZ): time(Y) & Y==2
	<- -captainpos(_);
		.my_name(ME);
		-+time(3);
		-+posObjective(CAPTAINPOSX+26,0,CAPTAINPOSZ-11);
		!add_task(task(3000,"TASK_GOTO_POSITION",ME,pos(CAPTAINPOSX+26,0,CAPTAINPOSZ-11),""));
		!add_task(task(2999,"TASK_GOTO_POSITION2",ME,pos(CAPTAINPOSX+27,0,CAPTAINPOSZ-10),""));
		-+state(standing).

+captainpos(CAPTAINPOSX,0,CAPTAINPOSZ): time(Y) & Y==1
	<- -captainpos(_);
		.my_name(ME);
		?time(TIME);
		-+time(TIME+1);
		-+posObjective(CAPTAINPOSX+24,0,CAPTAINPOSZ);
		!add_task(task(3000,"TASK_GOTO_POSITION",ME,pos(CAPTAINPOSX+24,0,CAPTAINPOSZ),""));
		-+state(standing).

+captainpos(CAPTAINPOSX,0,CAPTAINPOSZ)
	<- -captainpos(_);
		.my_name(ME);
		?time(TIME);
		-+time(TIME+1);
		-+posObjective(CAPTAINPOSX+24,0,CAPTAINPOSZ+12);
		!add_task(task(3000,"TASK_GOTO_POSITION",ME,pos(CAPTAINPOSX+24,0,CAPTAINPOSZ+12),""));
		-+state(standing).
