    ...
    while (bucle(X) & (X < Length)) {
        ...
        if (Type > 1000) {
            ...
            if (Team == 100) {  // Only if I'm ALLIED
                .nth(6,Object,Position); // Conocer la posición del agente a seguir //
                .my_name(ME);
                +Position;
                ?pos(XPOS,YPOS,ZPOS);
                !safe_pos(XPOS+UMBRALX,YPOS,ZPOS+UMBRALZ);
                ?safe_pos(XPOSAFE,YPOSAFE,ZPOSAFE);
                !add_task(task(3000,"TASK_GOTO_POSITION1",ME,pos(XPOSAFE,YPOSAFE,ZPOSAFE),""));      
                break;       
            }
...
+!update_targets
	<-
	  ...
	  ?vez(VEZ);
	  if(VEZ==1){
		!add_task(task(1000,"TASK_GOTO_POSITION",ME,pos(50,0,30),"")); // Todos se muevan a un punto y a partir de ahí sigan al atacante //
		-+vez(2);
	  }.
