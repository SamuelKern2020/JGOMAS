...
/////////////////////////////////
//  LOOK RESPONSE - triggered when the belief "look_response" is updated
/////////////////////////////////
...
* 
*/
+!perform_aim_action //DONT USE THIS
...
/**
     ...
     * 
     */
+!perform_injury_action . // - triggered when an agent is shot. We might want to use this method to have the agent look around and find the enemy who is shooting and then fire back
...
+!setup_priorities
    <-  +task_priority("TASK_NONE",0);
        ...
        +task_priority("TASK_GET_OBJECTIVE",1000);	//automatically added as the first task when agents are created, then when agent as the flag, changes the position to the home base (brings the flag back)
...
/////////////////////////////////
//  UPDATE TARGETS - triggers whenever the agent returns to the standing state. Use this to update tasks and their priorities
/////////////////////////////////
/**
...
+!performThresholdAction
       ...
       if (Hr <= Ht) { // - we might want to have the agent run away towards the nearest medic if the health threshold is reached
          ?my_position(X, Y, Z);
...
+cfm_agree[source(M)] // - triggered when a medic has confirmed the agent's request for medicpacs and is on its way
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR cfm_agree GOES HERE.")};
      -cfm_agree.  

+cfa_agree[source(M)] // - triggered when a Field-Ops agent has accepted the agent's request for amo and is on its way
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR cfa_agree GOES HERE.")};
      -cfa_agree.  

+cfm_refuse[source(M)]// - triggered when a medic has denied the agent's request for medicpacs and will not be coming to help
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR cfm_refuse GOES HERE.")};
      -cfm_refuse.  

+cfa_refuse[source(M)] // - triggered when a Field-Ops has denied the agent's request for medicpacs and will not be coming to help
...
//////////////////////////
//  Initialize variables - put the beliefs and objectives you want the agent to start out with here
/////////////////////////
