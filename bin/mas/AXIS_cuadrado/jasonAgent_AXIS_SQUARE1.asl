...
+!setup_priorities
    <-  +task_priority("TASK_NONE",0);
        ...
        +task_priority("TASK_GOTO_WELLPOSITION", 740);.   
...
+!init
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR init GOES HERE.")};
		...
		+pos_original_flag(INITIALFLAGX,INITIALFLAGY,INITIALFLAGZ);
		.my_name(ME);
		!add_task(task(3000,"TASK_GOTO_POSITION",ME,pos(INITIALFLAGX+4,0,INITIALFLAGZ+4),""));
		!add_task(task(2000,"TASK_GOTO_LOOKPOSITION",ME,pos(INITIALFLAGX+5,0,INITIALFLAGZ+5),"")).
