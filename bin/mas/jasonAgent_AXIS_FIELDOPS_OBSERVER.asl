...
+!init
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR init GOES HERE.")};
		...
		.my_name(ME);
		!add_task(task(3000,"TASK_GOTO_POSITION",ME,pos(INITIALFLAGX+6,0,INITIALFLAGZ),""));
		!add_task(task(2000,"TASK_GOTO_LOOKPOSITION",ME,pos(INITIALFLAGX+5,0,INITIALFLAGZ),"")).  
