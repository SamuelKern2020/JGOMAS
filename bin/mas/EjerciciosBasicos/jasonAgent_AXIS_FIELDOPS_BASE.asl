...
+!update_targets 
	...
	if(TIME==1){.send_msg_with_conversation_id(CAPTAIN, tell, Soldier_Arrived, "OKE");}
	else{
		if(TIME==2){.send_msg_with_conversation_id(CAPTAIN, tell, Soldier_Arrived, "OKE");}
		else{
		if(TIME==3){
			Soldier_final = "ready_base(1)";
			.send_msg_with_conversation_id(CAPTAIN, tell, Soldier_Arrived, "OKE");
			.send_msg_with_conversation_id(CAPTAIN, tell, Soldier_final, "OKY");
			}	
		}
	}.
...
+captainpos(CAPTAINPOSX,0,CAPTAINPOSZ): time(Y) & Y==2
	<- -captainpos(_);
		.my_name(ME);
		!add_task(task(3000,"TASK_GOTO_POSITION",ME,pos(CAPTAINPOSX-1,0,CAPTAINPOSZ-10),""));
		!add_task(task(3000,"TASK_GOTO_POSITION2",ME,pos(CAPTAINPOSX,0,CAPTAINPOSZ-10),""));
		.state(standing);
		-+time(3).
...
+captainpos(CAPTAINPOSX,0,CAPTAINPOSZ)
	<- -captainpos(_);
		.my_name(ME);
		?time(TIME);
		!add_task(task(3000,"TASK_GOTO_POSITION",ME,pos(CAPTAINPOSX-4,0,CAPTAINPOSZ-2),""));
		.state(standing);
		-+time(TIME+1).
