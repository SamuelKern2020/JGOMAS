...
+!update_targets 
	<-	?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR UPDATE_TARGETS GOES HERE.") };
		...
		if(TIME == 0){
			-+time(1);
			-+formed(FORMED+1);
			.concat("captainpos(",25,",0,",225,")",Captain_Position);
			.send_msg_with_conversation_id(AxisTeam, tell, Captain_Position, "FMT");
			!add_task(task(3000,"Task_GOTO_POSITION",ME,pos(25,0,210),""));
		}
		else{
			if(TIME==1 & FORMED==2 & TODOSFORMADOS=false){
				.concat("captainpos(",25,",0,",60,")",Captain_Position);
				.send_msg_with_conversation_id(AxisTeam, tell, Captain_Position, "FMT");
				-+formed(1);
				-+time(2);
				!add_task(task(3000,"Task_GOTO_POSITION",ME,pos(25,0,60),""));
			}
			else{
				if(TIME==2 & FORMED==2 & TODOSFORMADOS=false){
					BaseENEMIGA = "base(true)";
					.send_msg_with_conversation_id(AxisTeam, tell, BaseENEMIGA , "BAS");
					.concat("captainpos(",60,",0,",30,")",Captain_Position);
					.send_msg_with_conversation_id(AxisTeam, tell, Captain_Position , "BAS");
					!add_task(task(3000,"Task_GOTO_POSITION",ME,pos(59,0,30),""));
					!add_task(task(2999,"Task_GOTO_POSITION2",ME,pos(60,0,30),""));	
					-+time(3);
				}
			}
		}.
...
+ready_base(X)[source(A)]: formed(X) & X==8 <-
	-ready_base(_);
    -+todos_formados(true).
    
+ready_base(X)[source(A)] <-
    -ready_base(_).
